...
namespace TICO.GAUDI.Commons
{
    ...
    internal partial class ApplicationEngine : IApplicationEngineInternal
    {
        ...
        public async Task<bool> Init()
        {
            ...
            try
            {
                MyModuleClient = await ModuleClientFactory.CreateAsync().ConfigureAwait(false);
            }
            catch (Exception ex)
            {
            ...
            if (true == status)
            {
                status = await DesiredPropertiesUpdate(desiredProperties, null);
            ...
            if (true == status)
            {
                ...
                MyLogger.WriteLog(ILogger.LogLevel.INFO, $"Connection status change handler was set.");
            }
        ...
        {
            MyLogger.WriteLog(ILogger.LogLevel.TRACE, $"Start Method: Ready");
            ...
            if (ApplicationStateChangeResult.Ignored == stateChangeResult)
            {
                MyLogger.WriteLog(ILogger.LogLevel.ERROR, $"Exit Method: Ready caused by failed to change the ApplicationState to 'Ready'.", true);
            ...
            MyLogger.WriteLog(ILogger.LogLevel.TRACE, $"End Method: Ready");
            return status;
        }
        ...
        {
            MyLogger.WriteLog(ILogger.LogLevel.TRACE, $"Start Method: End");

            var status = true;

            // Endステートに遷移
            var stateChangeResult = Enqueue(ApplicationState.End);
            if (ApplicationStateChangeResult.Ignored == stateChangeResult && false == forced)
            {
                MyLogger.WriteLog(ILogger.LogLevel.ERROR, $"Exit Method: End caused by failed to change the ApplicationState to 'End'.", true);
                throw new Exception("Error: Failed to change the ApplicationState to 'End'.");
            }

            // アプリケーションのキャンセル
            if ( null != engineCanceller ) {
            ...
            MyLogger.WriteLog(ILogger.LogLevel.TRACE, $"End Method: End");
            return status;
        }
        ...
        public async Task<bool> Term(bool forced = false)
        {
            ...
            if (ApplicationStateChangeResult.Ignored == stateChangeResult && false == forced)
            {
                MyLogger.WriteLog(ILogger.LogLevel.TRACE, $"Exit Method: Term caused by failed to change the ApplicationState to 'Terminate'.");
        ...
        public async Task<bool> Restart()
        {
            ...
            if (false == result)
            {
                MyLogger.WriteLog(ILogger.LogLevel.ERROR, $"Exit Method: Restart caused by failed to stand-by ApplicationEngine.", true);
                throw new Exception("Error: Failed to change the ApplicationState to 'Ready'.");
            }
        ...
        /// エンジンの終了(internal)
        /// </summary>
        public async Task<bool> Terminate(bool forced = false)
        {
            MyLogger.WriteLog(ILogger.LogLevel.TRACE, $"Start Method: Terminate");

            var status = true;

            // エンジンの開放処理
            var result = await Term(forced);
            // （処理ステータスは無視）

            // エンジンの停止処理
            result = await End(forced);
            // （処理ステータスは無視）

            MyLogger.WriteLog(ILogger.LogLevel.TRACE, $"End Method: Terminate");
            return status;
        }
                                            ...
                                            object userContext)
        {
            ...
            if (false == result)
            {
                // Restart失敗時は、強制的に終了する。
                bool forced = true;
                await Terminate(forced);
            }
