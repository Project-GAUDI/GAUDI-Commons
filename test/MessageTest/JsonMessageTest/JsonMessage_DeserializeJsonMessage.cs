...
public class JsonMessage_DeserializeJsonMessage : JsonMessageTesterBase
{
    [Fact(DisplayName = "Message No071_正常終了する")]
    public void Message_Case071()
    {
        var input = CreateDummyMessage();
        var serialized = JsonConvert.SerializeObject(input);

        var result = JsonMessage.DeserializeJsonMessage(serialized);
        Assert.IsType<JsonMessage>(result);

        var expectedRecords = input.RecordList;
        Assert.Equal(expectedRecords.Count, result.RecordList.Count);
        for (var i = 0; i < expectedRecords.Count; ++i)
        {
            var expectedHeaders = expectedRecords[i].RecordHeader;
            var expectedData = expectedRecords[i].RecordData;

            Assert.Equal(expectedHeaders.Count, result.RecordList[i].RecordHeader.Count);
            Assert.Equal(expectedData.Count, result.RecordList[i].RecordData.Count);
            for (var j = 0; j < expectedHeaders.Count; ++j)
            {
                Assert.Equal(expectedHeaders[j], result.RecordList[i].RecordHeader[j]);
            }
            for (var j = 0; j < expectedData.Count; ++j)
            {
                Assert.Equal(expectedData[j], result.RecordList[i].RecordData[j]);
            }
        }
    }

    [Fact(DisplayName = "Message No072_例外が発生する")]
    public void Message_Case072()
    {
        var input = CreateDummyMessage();
        var serialized = JsonConvert.SerializeObject(input);
    ...
    JsonMessage? resultJsonMessage = null;
    JsonMessage.RecordInfo? resultRecordInfo = null;
